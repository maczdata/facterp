...
<html  lang="en">
    ...
    <head>
        ...
        <title> Facteezo: Purchase Order</title>
...
<!--<link rel="stylesheet" type="text/css" href="<?= base_url() ?>assets/widgets/datatable/datatable.css">-->
                ...
                $('.dataTables_filter input').attr("placeholder", "Search...");
                            ...
                            <div id="page-title" style="overflow: hidden;">
                                <div class="col-md-8">
                                    ...
                                    <p>Add, Edit or Delete Purchase Order</p>
                                </div>
                                ...
                                <div class="col-md-2">
                                    <button class="btn btn-blue-alt pull-right" onclick="AddPurchase();"><span style="margin-right: 5px;" class="glyph-icon icon-plus"></span>New Purchase Order</button>
                                </div>
                            </div>
                            <div id="collapseOne" class="panel-collapse collapse" aria-expanded="false">
                                <div class="panel-body">
                                    <div class="row" class="select_date">
                                        <form name="AccountForm" method="post" id="AccountForm" action="<?= base_url() ?>Purchase_order/" class="form-horizontal">
                            ...
                            <div class="panel">
                                <div class="panel-body">
                                    <?php
                                    $total_balnc = 0;
                                    foreach ($total_Purchase as $balnc) {
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="col-md-3">
                                                <a href="<?= base_url() ?>Purchase_order"> <h3 class="title-hero">
                                                        All Purchase Order
                                                    </h3></a>
                                            </div>
                                            <!-- <div class="col-md-7">
                                        ...
                                        <table class="table  table-bordered responsive no-wrap" cellspacing="0" width="100%">
                                            <tbody class="post-list" id="postList">
                                                ...
                                                if (!empty($Purchases)): $count = 1;
                                                        ...
                                                        <tr <?=(!empty($Purchase['invoice_id']) ? "style='background:#d5fdd5'":"")?>>
                                                            ...
                                                            <td>
                                                                <?php if ($this->session->userdata('user_group_id') == 1) { ?>
                                                                    <button class="btn btn-round btn-info" data-toggle="tooltip" data-placement="top" title="Edit" data-toggle="modal" data-target="#myModal" onclick="EditPurchaseOrder('<?= $Purchase['ordr_id'] ?>');">
                                                                        <i class="glyph-icon icon-pencil"></i>
                                                                    </button>
                                                                    <button class="btn btn-round btn-danger" data-toggle="tooltip" data-placement="top" title="Delete" onclick="DeletePurchaseOrder('<?= $Purchase['ordr_id'] ?>');">
                                                                        <i class="glyph-icon icon-trash"></i>
                                                                    </button>
                                                                <?php } ?>
                                                                <button class="btn btn-round btn-success" data-toggle="tooltip" data-placement="top" title="View" onclick="ViewPurchaseOrder('<?= $Purchase['ordr_id'] ?>');">
                                                                    <i class="glyph-icon icon-file-text-o"></i>
                                                                </button>

                                                                <button class="btn btn-round btn-warning" data-toggle="tooltip" data-placement="top" title="ConvertTOPurchase" onclick="ConvertPurchase('<?= $Purchase['ordr_id'] ?>');">
                                                    ...
                                                    ?>
                                                <p>Purchase(s) Order not available.</p>
                                            <?php endif; ?>
                                            <?php
                                            if (isset($search)) {
            ...
            <script>
                                                            function EditPurchaseOrder(ordr_id) {
                                                                window.location.href = "<?= base_url() ?>Purchase_order/view/" + ordr_id;
                                                            }
                                                            function PurchaseReturn(ordr_id) {
                                                                window.location.href = "<?= base_url() ?>Purchase/view_return/" + ordr_id;
                                                            }
                                                            function DeletePurchaseOrder(ordr_id) {
                                                                var cnfirm = confirm("Are You Sure?");
                                                                if (cnfirm) {
                                                                    $.post("<?= base_url() ?>Purchase_order/delete", {id: ordr_id})
                                                                            .done(function (data) {
                                                                                   // alert(data);
                                                                                if ($.trim(data) == 'done') {
                                                            ...
                                                            function ConvertPurchase(ordr_id) {
                                                                // alert("hello");
                                                                window.location.href = "<?= base_url() ?>Purchase_order/ViewConvertPurchase/" + ordr_id;
                                                            }
...
//                                                                        alert("hello");
                                                            ...
                                                            function ViewPurchaseOrder(ordr_id) {
                                                                // alert("hello");
                                                                window.location.href = "<?= base_url() ?>Purchase_order/ViewPurchaseOrder/" + ordr_id;
                                                            }
                                                            function GetProducts(cat_id) {

                                                                var base_url = "<?= base_url() ?>";
                                                                $("#all_pro_outer select").html("<option value=''>Select Option</option>");
                                                                $.post(base_url + "Purchase_order/GetProducts", {cat_id: cat_id})
                                                                        .done(function (data) {
                                                                            console.log(data);
                ...
                $("#search").keyup(function (event) {
                    ...
                    if (keycode == '13') {
                        ...
                        if (rtngPtId == "") {
                            ...
                            $.post("<?= base_url() ?>Purchase_order/search", {id: rtngPtId})
                                    .done(function (data) {
                                        console.log(data);
